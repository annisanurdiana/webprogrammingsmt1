//----------- THE MAIN PROJECT IN MY FINAL PROJECT ----------//
//---------------- SODOKU JS CODE -------------------------//
...
// User interface comunication functions
...
function checkButtonClick() {
    ...
    if (gameOn) {
        ...
        // if all values are correct and they are not equal original values then game over but the puzzle has not been solved yet
        if (currects === 81) {
            gameOn = false;
            pauseTimer = true;
    ...
    if (gameOn) {
        ...
        if (empty_cells_list.length === 0 && wrong_cells_list.length === 0) {
            ...
            if ((Math.random() < 0.5 && empty_cells_list.length > 0) || wrong_cells_list.length === 0) {
                ...
                input = table.rows[empty_cells_list[index][0]].cells[empty_cells_list[index][1]].
...
// show more option menu (hint dropdown)
...
function sudokuSolverMenuClick() {
    ...
    // show solve and check unique buttons and hide other buttons
